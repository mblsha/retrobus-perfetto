...
int main() {
  ...
  auto io_event =
      builder.add_instant_event(io_thread, "Port Write", timestamp - 500);
  io_event.add_annotation("port", 0x10).add_annotation("value", 0xFF);
  ...
  auto flow_start =
      builder.add_flow(cpu_thread, "DMA Request", timestamp, flow_id);
  flow_start.add_annotation("address", 0x8000).add_annotation("size", 256);

  // Complete flow on I/O thread
  timestamp += 2000;
  auto flow_end =
      builder.add_flow(io_thread, "DMA Complete", timestamp, flow_id, true);
