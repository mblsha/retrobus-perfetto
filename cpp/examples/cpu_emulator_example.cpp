// cpu_emulator_example.cpp - Example of integrating retrobus_perfetto with a
// CPU emulator
...
class Z80Emulator {
  ...
  uint64_t cycles_to_ns(uint64_t cycles) { return cycles * 250; }
  ...
  void execute_instruction(uint8_t opcode, int& memory_accesses) {
    switch (opcode) {
      ...
      case 0xCD: {  // CALL nn
        ...
        auto flow = trace_builder.add_flow(
            cpu_thread, "CALL", cycles_to_ns(cpu.cycle_count), flow_id);
      ...
      case 0xC9: {  // RET
        ...
        trace_builder.add_flow(cpu_thread, "RET", cycles_to_ns(cpu.cycle_count),
                               flow_id, true);
        break;
      }
