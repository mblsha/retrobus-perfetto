...
class Writer {
 public:
  Writer(void* buffer, std::size_t buffer_bytes, const Config& config) noexcept
      : status_(rbct_writer_init(&writer_,
                                 buffer,
                                 buffer_bytes,
                                 scopes_.data(),
                                 scopes_.size(),
  ...
  [[nodiscard]] bool enabled() const noexcept {
    return rbct_writer_enabled(&writer_) != 0;
  }
  [[nodiscard]] Status status() const noexcept { return status_; }

  Status begin(std::uint64_t timestamp,
               std::uint32_t track,
               std::uint32_t event,
  ...
  Status emit(std::uint64_t timestamp,
              std::uint32_t track,
              std::uint32_t event,
              std::initializer_list<Argument> arguments = {}) noexcept {
  ...
  Status clock_sync(std::uint32_t generation,
                    std::uint64_t before,
                    std::uint64_t after,
  ...
  [[nodiscard]] const void* data() const noexcept {
    return rbct_writer_data(&writer_);
  }
  [[nodiscard]] std::size_t size() const noexcept {
    return rbct_writer_size(&writer_);
  }
...
class TraceScope {
 public:
  TraceScope(Writer<ScopeDepth>* writer,
             Clock clock,
             std::uint32_t track,
             std::uint32_t event,
             std::initializer_list<Argument> arguments = {}) noexcept
