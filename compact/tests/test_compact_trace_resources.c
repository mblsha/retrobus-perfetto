...
STATIC_ASSERT(single_byte_frames, RBCT_RECORD_FRAME_BYTES == 1u);
STATIC_ASSERT(maximum_record_body, RBCT_MAX_RECORD_BYTES == 70u);
...
int main(void) {
  ...
  for (index = 0u; index < 4048u; ++index) {
    CHECK(rbct_writer_emit(&writer, index, 0u, 1u, NULL, 0u) == RBCT_OK);
  }
  ...
  CHECK(chunk[RBCT_CHUNK_HEADER_BYTES] == 3u * RBCT_MAX_RECORD_BYTES + 2u);
  CHECK(chunk[RBCT_CHUNK_HEADER_BYTES + 1u] ==
        3u * RBCT_MAX_RECORD_BYTES + 24u);

  CHECK(rbct_writer_emit(&writer, 4048u, 0u, 1u, NULL, 0u) == RBCT_OK);
  ...
  CHECK(rbct_writer_init(&writer, buffer, sizeof(buffer), &scope, 1u,
                         &config) == RBCT_OK);
  CHECK(rbct_writer_emit(&writer, 0u, 0u, 0u, NULL, 0u) == RBCT_OK);
  ...
  chunk = buffer + RBCT_FILE_HEADER_BYTES;
  CHECK(get_u16(chunk, 28u) == 72u);
  CHECK(chunk[RBCT_CHUNK_HEADER_BYTES + 1u] == RBCT_MAX_RECORD_BYTES);
  return 0;
}
