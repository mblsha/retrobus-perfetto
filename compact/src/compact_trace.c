...
static void rbct_copy(uint8_t *destination, const uint8_t *source, size_t size) {
    size_t index;
    for (index = 0; index < size; ++index) {
...
static int rbct_encode_event(const rbct_writer_t *writer,
    ...
    *encoded_size = 0u;
    ...
    if (!rbct_append_varint(encoded, capacity, encoded_size,
                            rbct_clock_delta(writer, timestamp,
...
rbct_status_t rbct_writer_init(rbct_writer_t *writer,
    ...
    if (writer == NULL || buffer == NULL || config == NULL ||
        ...
        config->clock_rate_denominator == 0u ||
        config->clock_width_bits == 0u || config->clock_width_bits > 64u ||
...
rbct_status_t rbct_writer_end(rbct_writer_t *writer, uint64_t timestamp) {
    ...
    if (writer == NULL || !rbct_writer_enabled(writer) || writer->scope_depth == 0u) {
        if (writer != NULL && rbct_writer_enabled(writer)) {
            ++writer->dropped_records;
        }
        return RBCT_INVALID_STATE;
    ...
    --writer->scope_depth;
    scope = &writer->scopes[writer->scope_depth];
    return rbct_append_event(writer, timestamp, scope->track_id, scope->event_id,
                             1, rbct_clock_delta(writer, timestamp,
                                                 scope->started_at),
...
rbct_status_t rbct_writer_clock_sync(rbct_writer_t *writer,
    ...
    if (!rbct_append_varint(encoded, sizeof(encoded), &encoded_size, generation) ||
        !rbct_append_varint(encoded, sizeof(encoded), &encoded_size, counter_before) ||
        !rbct_append_varint(encoded, sizeof(encoded), &encoded_size, counter_after) ||
...
rbct_status_t rbct_writer_finalize(rbct_writer_t *writer) {
    ...
    if (writer->valid_chunks == writer->chunk_count && writer->next_sequence > writer->chunk_count) {
